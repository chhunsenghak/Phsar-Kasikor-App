...
class _MarketplaceHomeScreenState extends State<MarketplaceHomeScreen> {
  ...
  Widget _buildViewToggleHeaders() {
    return Row(
            ...
            child: Container(
              ...
              child: Stack(
                children: [
                  ...
                  // Verification indicator trust badge
                  Positioned(
                    top: 8,
                    right: 8,
                    child: TrustBadge(
  ...
  Widget _buildProductList(BuildContext context, AppState state, List<MarketProduct> filtered) {
    if (filtered.isEmpty) {
      return Expanded(
        child: NoResultsScreen(onReset: () {
          state.setSearchQuery('');
    ...
    return Expanded(
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          final product = filtered[index];
          return _buildProductListCard(context, product);
        },
      ),
    );
  }

  Widget _buildProductListCard(BuildContext context, MarketProduct product) {
    final state = Provider.of<AppState>(context, listen: false);
    return CustomCard(
      padding: EdgeInsets.zero,
      onTap: () {
